...
${mermaidDiagrams && mermaidDiagrams.length > 0 ? 
  mermaidDiagrams.map((d: any) => `${d.title}: ${d.description}`).join('\n') : 
...
Please provide a comprehensive analysis focusing on architectural insights and best practices.`;
     ...
     */
    static generateDirectoryAnalysisPrompt(data: DirectoryPromptData): string {
...
**IMPORTANT**: The above business requirements are CRITICAL and must be given special attention. Please ensure the analysis addresses these requirements specifically, especially regarding shared-item logic layers, UI-logic decoupling architecture, and directory structure completeness.`
        ...
        return `You are a professional code analyst. Please generate a comprehensive analysis report for the following directory or page route. You MUST strictly follow the markdown heading levels in the required output format section.
...
- You MUST strictly follow the exact markdown heading levels as provided in this template
- ALWAYS use ## (double hash) for directory headings, not # (single hash)
- Use ### (triple hash) for subsections within a directory analysis
- Use #### (quadruple hash) for file headings within a directory
- NEVER change the heading level structure or format
- This is CRITICAL for documentation parser compatibility

## 📁 详细文件夹 - ${dir.fullPath || dir.path}
