...
export interface DataFlowAnalysisData {
    ...
    pageFlows?: Array<{
        ...
        pageFile: string;       // 页面文件路径
        apiCalls?: any[];       // API调用列表
        stateAccess?: any[];    // 状态访问列表
        componentUsage?: any[]; // 组件使用列表
    }>;
...
`;
    ...
    private static extractDirectoryDependencies(directoryPath: string, componentRelations: ComponentRelationsData): string {
        if (!componentRelations || !componentRelations.dependencies) return '暂无依赖信息';
        ...
        // 分类处理依赖关系
        const outgoingDeps: string[] = []; // 该目录依赖的其他目录
        const incomingDeps: string[] = []; // 其他目录依赖该目录
