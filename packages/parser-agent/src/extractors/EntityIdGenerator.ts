...
import { SyntaxKind, FunctionDeclaration, ClassDeclaration, VariableStatement, Node } from 'ts-morph';
...
export class EntityIdGenerator {
  private static entityMap: Map<string, Map<string, BaseEntity>> = new Map();
  private static fileTypeCache: Map<string, Map<string, string>> = new Map();
  // 第一层缓存：父路径候选实体缓存，key为parentPath，value为该父路径下的候选实体数组
  private static parentPathCandidatesCache: Map<string, BaseEntity[]> = new Map();
  // 第二层缓存：最终结果缓存，key为"relativePath:entityName:isDefaultExport"，value为匹配到的实体ID
  private static finalResultCache: Map<string, string | null> = new Map();
  ...
  static generateId(
    ...
    entityNode?: Node,
    isJSXContext: boolean = false
  ): { id: string; type: string; idPrefix: string } {
    ...
    if (fileEntities) {
      ...
      if (filePath.endsWith('.vue')) {
        ...
        if (isDefaultExport) {
          ...
          if (fileNameEntity) return fileNameEntity.id;
      ...
      if (fs.existsSync(filePath)) {
        ...
        if (stat.isDirectory()) {
          ...
          if (entryFile) {
            ...
            // 对于目录的默认导出，使用入口文件名
            if (isDefaultExport) {
              finalEntityName = path.basename(entryFile, path.extname(entryFile));
            }
            
            // 尝试从入口文件的实体映射中查找
            const entryRelativePath = rootDir ? path.relative(rootDir, entryFile) : entryFile;
            ...
            if (entryFileEntities) {
              if (isDefaultExport) {
                ...
                const entryFileName = path.basename(entryFile, path.extname(entryFile));
                const entryFileNameEntity = entryFileEntities.get(entryFileName);
                if (entryFileNameEntity) return entryFileNameEntity.id;
  ...
  private static analyzeNodeType(
    ...
    if (node.getKind() === SyntaxKind.FunctionDeclaration) {
      const isComponent = TypeUtils.isComponentFunction(node as FunctionDeclaration, isJSXContext);
      return {
    ...
    if (node.getKind() === SyntaxKind.ClassDeclaration) {
      const isComponent = TypeUtils.isComponentClass(node as ClassDeclaration, isJSXContext);
    ...
    if (node.getKind() === SyntaxKind.VariableStatement) {
      ...
      if (declarations.length > 0) {
        ...
        const isComponent = TypeUtils.isComponentVariable(declaration, isJSXContext);
  ...
  private static analyzeEntityTypeFromFile(
    ...
    try {
      ...
      const tsProjectManager = getTSProjectManager();
      ...
      let result: { id: string; type: string; idPrefix: string } | undefined;
      
      // 分析文件中的声明
      sf.forEachChild((node: Node) => {
        if (result) return; // 已找到结果
        
        if (isDefaultExport) {
          // 分析默认导出
          if (node.getKind() === SyntaxKind.FunctionDeclaration && (node as any).hasDefaultKeyword()) {
            result = this.analyzeNodeType(node, entityName, isJSXContext!);
          } else if (node.getKind() === SyntaxKind.ClassDeclaration && (node as any).hasDefaultKeyword()) {
            result = this.analyzeNodeType(node, entityName, isJSXContext!);
          } else if (node.getKind() === SyntaxKind.VariableStatement && (node as any).hasDefaultKeyword()) {
            result = this.analyzeNodeType(node, entityName, isJSXContext!);
          ...
          if (node.getKind() === SyntaxKind.FunctionDeclaration) {
            ...
            if (funcNode.getName() === entityName && (funcNode as any).hasExportKeyword()) {
              result = this.analyzeNodeType(node, entityName, isJSXContext!);
            }
          } else if (node.getKind() === SyntaxKind.ClassDeclaration) {
            const classNode = node as ClassDeclaration;
            if (classNode.getName() === entityName && (classNode as any).hasExportKeyword()) {
              result = this.analyzeNodeType(node, entityName, isJSXContext!);
            }
          } else if (node.getKind() === SyntaxKind.VariableStatement) {
            const varStatement = node as VariableStatement;
            if ((varStatement as any).hasExportKeyword()) {
              const declarations = varStatement.getDeclarations();
  ...
  private static searchInParentPaths(
    ...
    for (let i = pathParts.length - 1; i >= 0; i--) {
      const parentPath = i === 0 ? '' : pathParts.slice(0, i).join(path.sep);
   ...
   */
  private static getParentPathCandidates(parentPath: string): BaseEntity[] {
    // 第一层缓存：检查父路径候选实体缓存
    ...
    for (const [filePath, entities] of this.entityMap) {
      ...
      if (parentPath === '') {
        ...
        // 子目录情况
        isInParentPath = filePath.startsWith(parentPath + path.sep) || filePath === parentPath;
      }
  ...
  private static findMatchingEntity(
    ...
    if (isDefaultExport) {
      ...
      // 查找文件名与实体名称匹配的实体
      for (const entity of candidates) {
        const fileName = path.basename(entity.file, path.extname(entity.file));
   ...
   * 查找目录的入口文件
   */
  private static findEntryFile(dirPath: string): string | null {
    // 尝试不同的入口文件名
   ...
   * 在文件中查找特定实体
   */
  private static findEntityInFile(sf: any, entityName: string): { id: string; type: string; idPrefix: string } | null {
    let result: { id: string; type: string; idPrefix: string } | null = null;
    
    sf.forEachChild((node: Node) => {
      if (result) return;
      
      if (node.getKind() === SyntaxKind.FunctionDeclaration && (node as any).getName() === entityName) {
        result = this.analyzeNodeType(node, entityName, false);
      } else if (node.getKind() === SyntaxKind.ClassDeclaration && (node as any).getName() === entityName) {
        result = this.analyzeNodeType(node, entityName, false);
      } else if (node.getKind() === SyntaxKind.VariableStatement) {
        const declarations = (node as any).getDeclarations();
        for (const decl of declarations) {
          if (decl.getName() === entityName) {
            result = this.analyzeNodeType(node, entityName, false);
            break;
