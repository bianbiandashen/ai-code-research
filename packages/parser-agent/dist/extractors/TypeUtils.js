...
class TypeUtils {
     ...
     * 检查函数是否是组件（适用于TS和TSX）
     */
    static isComponentFunction(funcNode, isJSXContext = false) {
     ...
     * 检查类是否是组件（适用于TS和TSX）
     */
    static isComponentClass(classNode, isJSXContext = false) {
     ...
     * 检查变量是否是组件（适用于TS和TSX）
     */
    static isComponentVariable(declaration, isJSXContext = false) {
     ...
     * 检查变量是否是函数
     */
    static isFunctionVariable(declaration) {
        const initText = declaration.getInitializer()?.getText() || '';
        // 检查是否是箭头函数或函数表达式
     ...
     * 检查变量是否是常量
     */
    static isConstantVariable(declaration) {
        const varName = declaration.getName();
        const initText = declaration.getInitializer()?.getText() || '';
        // 检查是否是常量命名规范（全大写下划线）
     ...
     * 根据判断结果获取实体类型和ID前缀
     */
    static getEntityTypeInfo(isComponent, isFunction, isConstant) {
        if (isComponent) {
     ...
     * 获取类实体类型和ID前缀
     */
    static getClassTypeInfo(isComponent) {
        return isComponent
