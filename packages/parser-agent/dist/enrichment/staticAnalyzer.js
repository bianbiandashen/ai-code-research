...
const EntityIdGenerator_1 = require("../extractors/EntityIdGenerator");
/**
 * 生成唯一的临时文件名
 */
...
class StaticAnalyzer {
    constructor(rootDir, entities) {
        // 缓存已分析过的文件中导出的实体
        this.entityExportsCache = new Map();
        // 缓存文件路径到实体类型的映射
        this.fileEntityTypeCache = new Map();
        // 跟踪创建的临时文件
        ...
        EntityIdGenerator_1.EntityIdGenerator.initEntityMap(entities);
        // 初始化workspace信息
     ...
     * 解析工作区模式到实际路径
     */
    resolveWorkspacePattern(pattern) {
        const results = [];
        try {
     ...
     * 展开glob模式（支持 *, **, /\*\*\/* 等复杂模式）
     */
    expandGlobPattern(pattern, basePath) {
        const results = [];
        // 预处理模式：标准化路径分隔符
     ...
     * 展开递归模式（包含 ** 的模式）
     */
    expandRecursivePattern(pattern, basePath) {
        const results = [];
     ...
     * 展开简单模式（只包含单层通配符 * 的模式）
     */
    expandSimplePattern(pattern, basePath) {
        const results = [];
        const parts = pattern.split('/');
     ...
     * 递归查找目录下的所有子目录
     */
    findAllDirectories(rootPath, includeRoot = false) {
        const results = [];
     ...
     * 过滤出有效的workspace包
     */
    filterValidWorkspacePackages(candidatePaths) {
        const validPackages = [];
     ...
     */
    isValidWorkspacePackage(dirPath) {
        try {
            // 1. 检查目录是否存在
     ...
     * 检查模块是否为workspace包
     */
    isWorkspaceModule(moduleSpecifier, resolvedPath) {
     ...
     * 添加临时文件到项目中用于分析
     */
    addTempSourceFile(fileName, content) {
        try {
            // 使用唯一的临时文件名，避免冲突
        ...
        try {
            ...
            if (entity.file.endsWith('.vue')) {
                try {
                    ...
                    console.warn(`提取Vue文件注释失败: ${error.message}`);
                }
            }
            else {
                // 非Vue文件，直接提取JS/TS注释
                return this.extractJSComments(sf, entity);
            }
            return undefined;
        }
        catch (error) {
            console.error(`提取注释失败: ${error.message}`);
            return undefined;
        }
    }
    /**
    ...
    extractJSComments(sf, entity) {
        try {
            ...
            for (const node of allNodes) {
                ...
                else if (ts_morph_1.Node.isExportAssignment(node)) {
                    // 对于默认导出，检查导出表达式的名称
                ...
                if (shouldMatch && nodeName === entity.rawName) {
                    ...
                    if (leadingComments.length > 0) {
                        return this.formatComments(leadingComments);
                    }
                    // 如果没有前导注释，尝试获取后置注释
                    const trailingComments = node.getTrailingCommentRanges();
                    if (trailingComments.length > 0) {
                        return this.formatComments(trailingComments);
                    }
                }
            }
            // 如果精确匹配失败，尝试查找export语句中的变量
            for (const node of exportDeclarations) {
                if (ts_morph_1.Node.isExportDeclaration(node)) {
            ...
            for (const matchIndex of matches) {
                ...
                if (commentMatches.length > 0) {
                    ...
                    if (/^\s*(export\s+)?(const|let|var|function|class)?\s*$/.test(textBetween)) {
                        ...
                        return this.formatCommentText(commentText);
                    }
                }
            }
        }
        catch (error) {
            console.warn(`提取JS/TS注释失败: ${error.message}`);
        }
        return undefined;
    }
    /**
    ...
    formatComments(comments) {
        return comments
            .map(comment => {
            const text = comment.getText();
            return this.formatCommentText(text);
        })
            .join('\n')
            .trim();
    }
     ...
     * 分析Vue文件
     */
    async analyzeVueFile(filePath, entity) {
        const content = fs.readFileSync(filePath, 'utf-8');
        const { descriptor } = (0, compiler_sfc_1.parse)(content);
        const result = {
            IMPORTS: [],
     ...
     * 分析TSX文件
     */
    analyzeTSXFile(filePath, entity) {
        const content = fs.readFileSync(filePath, 'utf-8');
     ...
     * 根据文件路径和导出名称生成实体ID
     */
    generateEntityId(filePath, exportName, isDefaultExport = false) {
        return EntityIdGenerator_1.EntityIdGenerator.generateIdByLookup(filePath, exportName, isDefaultExport, this.rootDir);
    }
    /**
     * 解析TypeScript路径别名（通用版本）
     */
    resolveTypeScriptAlias(moduleSpecifier, projectRoot) {
     ...
     * 检查模块是否为第三方库
     */
    isThirdPartyModule(moduleSpecifier) {
        // 第三方库特征：
        // 1. 不以 . 或 / 开头（相对路径）
        // 2. 不匹配任何配置的路径别名
        // 3. 不是workspace包
        ...
        // 检查是否为workspace包
        const isWorkspacePackage = this.workspaceInfo?.packageNames.some(pkgName => moduleSpecifier === pkgName || moduleSpecifier.startsWith(pkgName + '/'));
     ...
     * 提取导入声明
     */
    extractImports(sf, result, realFilePath) {
        const entityIds = [];
     ...
     * 查找workspace包的真实路径
     */
    findWorkspaceRealPath(packageName) {
        if (!this.workspaceInfo) {
     ...
     * 解析重新导出的模块路径
     */
    resolveReExportModulePath(moduleSpecifier, currentFilePath) {
        try {
            // 使用现有的模块路径解析逻辑
     ...
     * 解析模块路径，获取绝对路径
     */
    resolveModulePath(moduleSpecifier, currentFilePath) {
        let resolvedPath = '';
        if (moduleSpecifier.startsWith('.')) {
            // 相对路径
            const dirPath = path.dirname(currentFilePath);
     ...
     * 分析模块导出的实体
     */
    analyzeModuleExports(modulePath) {
        // 检查缓存
        ...
        try {
            ...
            console.error(`分析模块导出失败 ${modulePath}: ${error.message}`);
        }
        // 缓存结果
     ...
     * 从源文件中提取导出的实体
     */
    extractExports(sf, exports, realFilePath) {
     ...
     * 提取 CommonJS 格式的导出
     */
    extractCommonJSExports(sf, exports) {
        const sourceText = sf.getFullText();
     ...
     * 提取函数调用
     */
    extractCalls(sf, result, currentFilePath) {
        const entityIds = [];
        ...
        sf.forEachDescendant(node => {
            if (ts_morph_1.Node.isCallExpression(node)) {
                ...
                // 处理元素访问调用，如 object['method']()
                    ...
                    if (argumentExpression && ts_morph_1.Node.isStringLiteral(argumentExpression) && workspaceImports.has(object)) {
     ...
     * 提取模板中的组件引用
     */
    extractTemplateComponents(template, result) {
    ...
    loadPathAliases() {
        ...
        try {
            ...
            console.warn(`加载路径别名配置失败: ${error.message}`);
        }
        return aliases;
    }
    /**
    ...
    loadTsConfigAliases(aliases) {
        try {
            ...
            console.warn(`从tsconfig.json加载路径别名失败: ${error.message}`);
        }
    }
    /**
     * 从 vue.config.js 加载路径别名
     */
    loadVueConfigAliases(aliases) {
        try {
            ...
            console.warn(`从vue.config.js加载路径别名失败: ${error.message}`);
        }
    }
    /**
     * 从 vite.config.js/ts 加载路径别名
     */
    loadViteConfigAliases(aliases) {
        try {
            ...
            console.warn(`从vite配置文件加载路径别名失败: ${error.message}`);
        }
    }
    /**
     * 从 webpack.config.js 加载路径别名
     */
    loadWebpackConfigAliases(aliases) {
        try {
            ...
            console.warn(`从webpack.config.js加载路径别名失败: ${error.message}`);
        }
    }
    /**
     * 查找 tsconfig.json 文件
    ...
    parseTsConfig(configPath) {
        try {
            const configContent = fs.readFileSync(configPath, 'utf-8');
            const config = JSON.parse((0, jsonrepair_1.jsonrepair)(configContent));
            // 处理 extends
            if (config.extends) {
            ...
            console.warn(`解析 tsconfig.json 失败: ${error.message}`);
            return {};
        }
    }
}
