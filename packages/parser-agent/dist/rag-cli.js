 ...
 * @param result 搜索结果
 */
function displaySearchSummary(result) {
    console.log(`找到 ${result.entities.length} 个相关组件`);
    console.log('相关性排序:');
    // 按相关度排序实体
    const sortedEntities = [...result.entities].sort((a, b) => (result.relevanceScores[b.id] || 0) - (result.relevanceScores[a.id] || 0));
    // 显示每个实体的基本信息
 ...
 * @param result 关系分析结果
 */
function displayRelationshipSummary(result) {
...
program
    ...
    .requiredOption('-e, --entities <path>', '实体JSON文件路径', path_1.default.join(process.cwd(), 'data/entities.enriched.json'))
    .option('-r, --root <path>', '项目根目录', process.cwd())
    .option('-o, --output <path>', '输出提示词的文件路径', path_1.default.join(process.cwd(), 'search/prompt.txt'))
...
`);
        // 执行搜索
        const result = await (0, rag_inline_tool_1.searchCodeEntities)(query, options.entities, options.root, parseInt(options.top, 10));
        // 显示搜索结果摘要
        displaySearchSummary(result);
        // 保存提示词到文件
        if (options.output) {
            ensureOutputDirectory(options.output);
            fs_1.default.writeFileSync(options.output, result.prompt, 'utf8');
...
program
    ...
    .requiredOption('-e, --entities <path>', '实体JSON文件路径', path_1.default.join(process.cwd(), 'data/entities.enriched.json'))
    .option('-r, --root <path>', '项目根目录', process.cwd())
    .option('-o, --output <path>', '输出提示词的文件路径', path_1.default.join(process.cwd(), 'related/prompt.txt'))
...
`);
        // 执行关系查询
        const result = await (0, rag_inline_tool_1.getRelatedCodeEntities)(entityId, options.entities, options.root, parseInt(options.max, 10));
        if (!result) {
...
program
    ...
    .requiredOption('-e, --entities <path>', '实体JSON文件路径', path_1.default.join(process.cwd(), 'data/entities.enriched.json'))
    .option('-r, --root <path>', '项目根目录', process.cwd())
    .option('-o, --output <path>', '输出提示词的文件路径', path_1.default.join(process.cwd(), 'search-related/prompt.txt'))
    .option('-k, --top <number>', '返回的组件数量', '5')
    .option('-m, --max <number>', '每种关系类型返回的最大相关实体数', '3')
    .action(async (query, options) => {
    try {
...
`);
        ...
        const searchResult = await (0, rag_inline_tool_1.searchCodeEntities)(query, options.entities, options.root, parseInt(options.top, 10));
        // 显示搜索结果摘要
        displaySearchSummary(searchResult);
        // 按相关度排序搜索结果
        const sortedEntities = [...searchResult.entities].sort((a, b) => (searchResult.relevanceScores[b.id] || 0) - (searchResult.relevanceScores[a.id] || 0));
        // 第二步：对每个搜索结果执行二跳查询
        console.log('\n步骤2: 执行关系分析...');
        const allResults = [];
        for (const entity of sortedEntities) {
            console.log(`正在查询 ${entity.id} 的相关组件...`);
            const relatedResult = await (0, rag_inline_tool_1.getRelatedCodeEntities)(entity.id, options.entities, options.root, parseInt(options.max, 10));
            if (relatedResult) {
                allResults.push({
                    searchEntity: entity,
        ...
        if (options.output) {
            ensureOutputDirectory(options.output);
            fs_1.default.writeFileSync(options.output, combinedPrompt, 'utf8');
