...
exports.buildRealWorkspacePackageMap = buildRealWorkspacePackageMap;
exports.getWorkspaceInfo = getWorkspaceInfo;
exports.generateIntelligentSearchPaths = generateIntelligentSearchPaths;
const path_1 = __importDefault(require("path"));
const fs_1 = __importDefault(require("fs"));
const util_1 = require("util");
const VueExtractor_1 = require("./extractors/VueExtractor");
const TSXExtractor_1 = require("./extractors/TSXExtractor");
const FunctionExtractor_1 = require("./extractors/FunctionExtractor");
...
const PATH_RESOLVE_CACHE = new Map();
const WORKSPACE_INFO_CACHE = new Map();
const WORKSPACE_PACKAGE_CACHE = new Map();
/**
 * 清理缓存以释放内存
 */
function clearCache() {
...
async function getCachedFileContent(filePath) {
    ...
    try {
        ...
        // 检查缓存大小限制
        const currentCacheSize = Array.from(FILE_CONTENT_CACHE.values())
...
function findWorkspaceRoot(startDir) {
    ...
    while (currentDir !== path_1.default.dirname(currentDir)) { // 直到根目录
 ...
 */
function getWorkspaceInfo(rootDir) {
    const packageNames = [];
    const packagePaths = [];
    ...
    try {
        ...
        if (fs_1.default.existsSync(packageJsonPath)) {
            const packageJson = JSON.parse(fs_1.default.readFileSync(packageJsonPath, 'utf-8'));
            // 收集所有workspace:*依赖的包名
            const collectWorkspaceDeps = (deps = {}) => {
        ...
        if (packageNames.length > 0) {
            ...
            console.log(`找到 ${packageNames.length} 个workspace包和 ${workspacePatterns.length} 个workspace模式`);
            ...
            for (const packageName of packageNames) {
                ...
                for (const pattern of workspacePatterns) {
                    ...
                    console.log(`  模式 ${pattern} 解析出 ${possiblePaths.length} 个路径`);
                    for (const possiblePath of possiblePaths) {
                        const possiblePackageJsonPath = path_1.default.join(possiblePath, 'package.json');
                        if (fs_1.default.existsSync(possiblePackageJsonPath)) {
                            try {
                                const possiblePackageJson = JSON.parse(fs_1.default.readFileSync(possiblePackageJsonPath, 'utf-8'));
                                if (possiblePackageJson.name === packageName) {
                                    packagePaths.push(possiblePath);
                                    console.log(`通过工作区配置找到: ${packageName} -> ${possiblePath}`);
                                    found = true;
                                    break;
                                }
                            }
                            catch (error) {
                                console.warn(`解析package.json失败: ${possiblePackageJsonPath}`, error);
                            }
                        }
                    }
                    if (found)
                ...
                if (!found) {
                    ...
                    const searchResult = searchPackageByIntelligentPatterns(workspaceRoot, packageName);
                    if (searchResult) {
                ...
                if (!found) {
                    ...
                    const nodeModulesPath = path_1.default.join(workspaceRoot, 'node_modules');
                    if (fs_1.default.existsSync(nodeModulesPath)) {
                        let packagePath;
                        // 处理作用域包和普通包
                        if (packageName.startsWith('@')) {
                            packagePath = path_1.default.join(nodeModulesPath, packageName);
                        }
                        else {
                            packagePath = path_1.default.join(nodeModulesPath, packageName);
                        }
                        try {
                            if (fs_1.default.existsSync(packagePath)) {
                                const stat = fs_1.default.lstatSync(packagePath);
                                // 如果是符号链接，解析到真实路径
                                if (stat.isSymbolicLink()) {
                                    const realPath = fs_1.default.realpathSync(packagePath);
                                    console.log(`  符号链接解析: ${packagePath} -> ${realPath}`);
                                    // 只有当真实路径不包含node_modules时才添加（说明是workspace包）
                                    if (!realPath.includes('node_modules')) {
                                        packagePaths.push(realPath);
                                        console.log(`通过符号链接找到: ${packageName} -> ${realPath}`);
                                        found = true;
                                    }
                                    else {
                                        // 处理pnpm的特殊情况
                                        const pnpmMatch = realPath.match(/node_modules\/\.pnpm\/file\+packages\+([^/]+)(?:\+([^/]+))*\/node_modules\/(@[^/]+\/[^/]+|[^/]+)/);
                                        if (pnpmMatch) {
                                            console.log(`  检测到pnpm路径，解析路径组件...`);
                                            // 尝试从pnpm路径推断workspace路径
                                            const workspacePath = inferWorkspacePathFromPnpm(workspaceRoot, realPath, packageName);
                                            if (workspacePath) {
                                                packagePaths.push(workspacePath);
                                                console.log(`通过pnpm路径推断找到: ${packageName} -> ${workspacePath}`);
                                                found = true;
                                            }
                                        }
                                    }
                            ...
                            console.warn(`处理workspace包 ${packageName} 时出错:`, error.message);
                        }
                    }
                }
                ...
                if (!found) {
                    ...
                    try {
                        const searchResult = recursiveSearchPackage(workspaceRoot, packageName, 3); // 限制深度为3
                        if (searchResult) {
                            packagePaths.push(searchResult);
                            console.log(`通过递归搜索找到: ${packageName} -> ${searchResult}`);
                            found = true;
                        }
                    }
                    catch (error) {
        ...
        // 如果根项目没有依赖，也需要扫描所有workspace路径
        // 这样可以确保扫描测试环境和开发环境
        if (packagePaths.length === 0 && workspaceRoot) {
            console.log('未找到任何workspace包路径，尝试扫描所有workspace路径');
            const workspacePatterns = getWorkspacePatterns(workspaceRoot);
            for (const pattern of workspacePatterns) {
                // 检查超时
                if (Date.now() - startTime > TOTAL_TIMEOUT_MS) {
                    console.warn(`扫描所有workspace路径时超时，停止处理`);
                    break;
                }
                const possiblePaths = resolveWorkspacePattern(workspaceRoot, pattern);
                packagePaths.push(...possiblePaths);
            }
            console.log(`未找到依赖声明，扫描所有workspace路径: ${packagePaths.length}个`);
        }
        const totalTime = Date.now() - startTime;
        console.log(`getWorkspaceInfo 完成，总耗时: ${totalTime}ms，找到 ${packagePaths.length} 个workspace路径`);
    }
    catch (error) {
...
function getWorkspacePatterns(rootDir) {
    ...
    if (fs_1.default.existsSync(pnpmWorkspacePath)) {
        try {
            ...
            console.log(`读取pnpm-workspace.yaml内容:`, yamlContent.substring(0, 200));
            // 更灵活的YAML解析
        ...
        try {
            ...
            if (packageJson.workspaces) {
                if (Array.isArray(packageJson.workspaces)) {
                    ...
                    console.log(`从package.json解析到数组模式: ${JSON.stringify(packageJson.workspaces)}`);
                }
                else if (packageJson.workspaces.packages) {
                    patterns.push(...packageJson.workspaces.packages);
                    console.log(`从package.json解析到packages模式: ${JSON.stringify(packageJson.workspaces.packages)}`);
                }
                    ...
                    Object.keys(packageJson.workspaces).forEach(key => {
                        if (Array.isArray(packageJson.workspaces[key])) {
                            ...
                            console.log(`从package.json解析到${key}模式: ${JSON.stringify(packageJson.workspaces[key])}`);
                        }
                    });
                }
            }
    ...
    if (patterns.length === 0) {
        ...
        console.log(`未找到workspace配置，使用默认模式: ${JSON.stringify(defaultPatterns)}`);
    ...
    try {
        ...
        if (pattern.includes('*')) {
            ...
            for (let i = 0; i < parts.length; i++) {
                ...
                const part = parts[i];
                console.log(`    处理部分 ${i}: ${part}, 当前路径数: ${currentPaths.length}`);
                if (part === '*') {
                    const newPaths = [];
                        ...
                        if (fs_1.default.existsSync(currentPath)) {
                            try {
                                ...
                                for (const entry of entries) {
                                    ...
                                    // 跳过常见的大目录和不需要的目录
                                    if (entry.startsWith('.') ||
                                        ['node_modules', 'dist', 'build', 'coverage', 'tmp', 'temp', '.git',
                                            'logs', 'cache', '.cache', 'public', 'static', 'assets'].includes(entry)) {
                                        continue;
                                    }
                                    ...
                                    try {
                                        if (fs_1.default.statSync(entryPath).isDirectory()) {
                                            ...
                                            if (processedCount >= MAX_DIRS_PER_LEVEL) {
                                                console.warn(`      达到目录数量限制 ${MAX_DIRS_PER_LEVEL}，停止处理更多目录`);
                                                break;
                                            }
                                        }
                                    }
                                ...
                                console.warn(`      读取目录失败: ${currentPath}, 错误: ${error.message}`);
                            }
                        }
                        else {
        ...
        console.log(`    模式 ${pattern} 解析完成，返回 ${results.length} 个路径，耗时 ${elapsedTime}ms`);
    }
    catch (error) {
...
function findAllDirectories(dir, maxDepth = 5, currentDepth = 0) {
    ...
    try {
        if (fs_1.default.existsSync(dir)) {
            ...
            console.log(`        递归扫描目录 ${dir} (深度 ${currentDepth}): ${entries.length} 个条目`);
            for (const entry of entries) {
                // 跳过常见的不需要扫描的目录
                if (entry.startsWith('.') ||
                    ['node_modules', 'dist', 'build', 'coverage', 'tmp', 'temp', '.git'].includes(entry)) {
                    continue;
                }
                const entryPath = path_1.default.join(dir, entry);
                try {
                    if (fs_1.default.statSync(entryPath).isDirectory()) {
                        directories.push(...findAllDirectories(entryPath, maxDepth, currentDepth + 1));
                    }
                }
                catch (error) {
                    // 忽略访问权限错误等
        ...
        console.log(`        递归扫描目录失败: ${dir}, 错误: ${error.message}`);
    }
    return directories;
}
/**
 ...
 */
function recursiveSearchPackage(rootDir, packageName, maxDepth = 3, currentDepth = 0) {
    // 防止递归过深
    if (currentDepth >= maxDepth) {
        return null;
    }
    try {
        ...
        const entries = fs_1.default.readdirSync(rootDir);
        for (const entry of entries) {
            // 跳过常见的不需要搜索的目录
            if (entry.startsWith('.') ||
                ['node_modules', 'dist', 'build', 'coverage', 'tmp', 'temp', '.git',
            ...
            const entryPath = path_1.default.join(rootDir, entry);
            try {
                ...
                if (stat.isDirectory()) {
                    ...
                    if (fs_1.default.existsSync(packageJsonPath)) {
                        try {
                            ...
                            if (packageJson.name === packageName) {
                                console.log(`    递归搜索找到包: ${packageName} 在 ${entryPath}`);
                                return entryPath;
                            }
                        }
                        catch (error) {
                    ...
                    // 递归搜索子目录
                    const result = recursiveSearchPackage(entryPath, packageName, maxDepth, currentDepth + 1);
                    if (result) {
                        return result;
                    }
                }
 ...
 */
function ensureUniqueIds(entities) {
    const idMap = new Map();
 ...
 * 并行扫描目录，查找指定扩展名的文件
 */
async function findFilesWithExtensionsAsync(dir, extensions) {
    const startTime = Date.now();
    const results = [];
    // 获取workspace信息（带缓存）
    ...
    if (packagePaths.length > 0) {
        ...
        const scanPromises = packagePaths.map(workspacePath => {
            ...
            return scanDirectoryAsync(workspacePath, extensions, PERFORMANCE_CONFIG.MAX_PARALLEL_DIRS);
        });
        // 添加项目根目录本身的源码目录扫描
        ...
        console.log('未找到workspace配置，使用默认扫描方式...');
        const defaultResults = await scanDirectoryAsync(dir, extensions, PERFORMANCE_CONFIG.MAX_PARALLEL_DIRS);
        results.push(...defaultResults);
    }
    // 去重
    const uniqueResults = Array.from(new Set(results));
    const scanTime = Date.now() - startTime;
    console.log(`📊 文件扫描完成: 找到 ${uniqueResults.length} 个文件，耗时 ${scanTime}ms`);
    return uniqueResults;
}
/**
 * 扫描项目根目录的常见源码目录
 */
function scanProjectRootDirectories(dir, extensions, results) {
    // 常见的项目源码目录
    const commonSourceDirs = ['src', 'lib', 'app', 'components', 'pages', 'views', 'utils'];
    // 先扫描根目录的直接文件
    scanRootFiles(dir, extensions, results);
    // 然后扫描常见的源码目录
    for (const sourceDir of commonSourceDirs) {
        const sourcePath = path_1.default.join(dir, sourceDir);
 ...
 * 异步扫描项目根目录的常见源码目录
 */
async function scanProjectRootDirectoriesAsync(dir, extensions) {
    const results = [];
    // 常见的项目源码目录
    const commonSourceDirs = ['src', 'lib', 'app', 'components', 'pages', 'views', 'utils'];
    ...
    const scanPromises = commonSourceDirs.map(async (sourceDir) => {
        ...
        try {
            ...
            if (stat.isDirectory()) {
                console.log(`📂 扫描项目源码目录: ${sourcePath}`);
                return await scanDirectoryAsync(sourcePath, extensions, PERFORMANCE_CONFIG.MAX_PARALLEL_DIRS);
            }
        }
        catch {
            // 目录不存在，忽略
 ...
 * 异步并行递归扫描目录
 */
async function scanDirectoryAsync(dir, extensions, maxParallel = 4) {
    const results = [];
        ...
        const batchPromises = currentBatch.map(async (currentDir) => {
            try {
                ...
                const filePromises = files.map(async (file) => {
                    if (file.startsWith('.') ||
                        ['node_modules', 'dist', 'build', 'coverage', 'tmp', 'temp'].includes(file)) {
                        return;
                    }
                    const currentPath = path_1.default.join(currentDir, file);
                    try {
                        const fileStat = await stat(currentPath);
                        if (fileStat.isDirectory()) {
                            subDirs.push(currentPath);
                        }
                        else if (extensions.includes(path_1.default.extname(file))) {
 ...
 * 扫描根目录下的直接文件（不递归）
 */
function scanRootFiles(dir, extensions, results) {
    try {
        ...
        for (const file of files) {
            ...
            // 跳过目录和特殊文件
            if (fs_1.default.statSync(currentPath).isDirectory() || file.startsWith('.')) {
                continue;
            }
 ...
 * 异步并行扫描根目录文件
 */
async function scanRootFilesAsync(dir, extensions) {
    try {
        console.log(`📁 扫描根目录文件: ${dir}`);
        ...
        const checkPromises = files.map(async (file) => {
            ...
            try {
                const fileStat = await stat(currentPath);
                if (!fileStat.isDirectory() && extensions.includes(path_1.default.extname(file))) {
                    return currentPath;
                }
            }
            catch {
        ...
        results.push(...checkedFiles.filter(Boolean));
...
// 判断是否是 DDD 实体
function isDDDEntity(fileContent) {
    // 检查是否导入了 @xhs/di
...
async function extractAllEntities(rootDir, targetFiles) {
    const startTime = Date.now();
    console.log(`🚀 开始从目录提取实体: ${rootDir}`);
    const extensions = ['.vue', '.ts', '.tsx'];
    // 使用异步文件扫描
    const allFiles = targetFiles
        ? targetFiles.filter(file => extensions.includes(path_1.default.extname(file)))
    ...
    for (let i = 0; i < allFiles.length; i += batchSize) {
        ...
        console.log(`📊 处理批次 ${Math.floor(i / batchSize) + 1}/${Math.ceil(allFiles.length / batchSize)} (${batch.length} 个文件)`);
        // 将批次分成更小的并行组
        ...
        for (const group of parallelGroups) {
            const groupPromises = group.map(async (file) => {
                try {
                    ...
                    console.error(`⚠️  提取文件 ${file} 失败: ${error.message}`);
                    return [];
                }
            });
        ...
        if ((i + batchSize) % (batchSize * 4) === 0) {
            const currentCacheSize = Array.from(FILE_CONTENT_CACHE.values())
                .reduce((sum, item) => sum + item.content.length, 0);
            if (currentCacheSize > PERFORMANCE_CONFIG.CACHE_SIZE_LIMIT) {
                console.log(`🧹 清理缓存，当前大小: ${Math.round(currentCacheSize / 1024 / 1024)}MB`);
                FILE_CONTENT_CACHE.clear();
            }
        }
    }
    ...
    console.log(`  - 平均耗时: ${Math.round(extractTime / allFiles.length)}ms/文件`);
    // 确保所有实体ID唯一
    const startUniqueTime = Date.now();
    const uniqueEntities = ensureUniqueIds(all);
    const uniqueTime = Date.now() - startUniqueTime;
    console.log(`🔄 ID唯一性处理: ${uniqueTime}ms, 最终实体数: ${uniqueEntities.length}`);
    // 最终清理缓存
    clearCache();
 ...
 * 判断文件是否来自workspace包
 */
function isWorkspaceFile(filePath, rootDir) {
    // 获取相对于rootDir的路径
 ...
 * 从单个文件提取实体（异步版本）
 */
async function extractEntitiesFromFile(file, rootDir) {
    let entities = [];
    try {
        ...
        if (normalizedFile.endsWith('.vue')) {
            ...
            entities = VueExtractor_1.VueExtractor.extract(normalizedFile, rootDir);
        }
        else if (normalizedFile.endsWith('.tsx')) {
            console.log(`📄 从TSX文件提取实体: ${normalizedFile}`);
            entities = TSXExtractor_1.TSXExtractor.extract(normalizedFile, rootDir);
        }
        else if (normalizedFile.endsWith('.ts')) {
            console.log(`📄 从TS文件提取实体: ${normalizedFile}`);
            entities = FunctionExtractor_1.FunctionExtractor.extract(normalizedFile, rootDir);
        }
        // 确保所有实体的file字段都使用相对路径，并添加标识
        entities = entities.map(entity => ({
            ...entity,
            file: path_1.default.relative(rootDir, normalizedFile),
            isWorkspace: isWorkspace,
            isDDD: isDDD || entity.isDDD || false
        }));
        console.log(`✅ 从文件 ${normalizedFile} 提取到 ${entities.length} 个实体${isWorkspace ? ' (workspace)' : ''}`);
        return entities;
    }
    catch (error) {
        console.error(`❌ 提取文件 ${file} 失败: ${error.message}`);
        throw error;
    }
}
// 根据文件路径规范化
// 如果是node_modules中的workspace包，转换为对应的真实路径
function normalizeWorkspacePath(filePath, rootDir) {
    ...
    try {
        ...
        const pnpmMatch = filePath.match(/node_modules\/\.pnpm\/file\+packages\+([^/]+)\/node_modules\/(@[^/]+\/[^/]+|[^/]+)\/(.+)$/);
        if (pnpmMatch) {
            ...
            console.log(`检测到pnpm链接包: ${packageName} (文件夹: ${packageFolder})`);
            // 尝试找到对应的workspace路径
            for (const wsPath of packagePaths) {
                if (wsPath.endsWith(`/packages/${packageFolder}`) || wsPath.endsWith(`\\packages\\${packageFolder}`)) {
                    const normalizedPath = path_1.default.join(wsPath, relativePath);
        ...
        if (nodeModulesMatch) {
            ...
            // 查找对应的workspace路径
            const packageIndex = packageNames.findIndex(name => name === packageName);
            if (packageIndex >= 0 && packageIndex < packagePaths.length) {
                const workspacePath = packagePaths[packageIndex];
                const normalizedPath = path_1.default.join(workspacePath, relativePath);
 ...
 * 通过智能包名模式搜索包
 */
function searchPackageByIntelligentPatterns(workspaceRoot, packageName) {
    console.log(`    智能搜索包: ${packageName}`);
    // 生成可能的搜索路径
    const searchPaths = generateIntelligentSearchPaths(workspaceRoot, packageName);
    for (const searchPath of searchPaths) {
        console.log(`    检查路径: ${path_1.default.relative(workspaceRoot, searchPath)}`);
        if (fs_1.default.existsSync(searchPath)) {
            const packageJsonPath = path_1.default.join(searchPath, 'package.json');
            if (fs_1.default.existsSync(packageJsonPath)) {
                try {
                    const packageJson = JSON.parse(fs_1.default.readFileSync(packageJsonPath, 'utf-8'));
                    if (packageJson.name === packageName) {
 ...
 */
function generateIntelligentSearchPaths(workspaceRoot, packageName) {
    const paths = [];
    // 移除作用域前缀
    const nameWithoutScope = packageName.startsWith('@') ? packageName.split('/')[1] : packageName;
    // 1. 保持原有的特定包硬编码逻辑（确保兼容性）
    if (packageName === '@xhs/shared-ark') {
        // shared-ark 可能在 shared 目录中
        paths.push(path_1.default.join(workspaceRoot, 'shared', 'shared-ark'), path_1.default.join(workspaceRoot, 'shared', 'ark'), path_1.default.join(workspaceRoot, 'packages', 'shared', 'shared-ark'), path_1.default.join(workspaceRoot, 'packages', 'shared', 'ark'), path_1.default.join(workspaceRoot, 'packages', 'shared-ark'));
    }
    else if (packageName === '@xhs/fulfillment-common-ark') {
        // fulfillment-common-ark 可能在 fulfillment/fulfillment-common 下
        paths.push(path_1.default.join(workspaceRoot, 'packages', 'fulfillment', 'fulfillment-common', 'ark'), path_1.default.join(workspaceRoot, 'packages', 'fulfillment', 'fulfillment-common-ark'), path_1.default.join(workspaceRoot, 'packages', 'fulfillment-common', 'ark'), path_1.default.join(workspaceRoot, 'packages', 'fulfillment-common-ark'));
    }
    else if (packageName === '@xhs/lib-fulfillment-modules') {
        // lib-fulfillment-modules 可能在 fulfillment/fulfillment-common 下
        paths.push(path_1.default.join(workspaceRoot, 'packages', 'fulfillment', 'fulfillment-common', 'biz-module'), path_1.default.join(workspaceRoot, 'packages', 'fulfillment', 'fulfillment-common', 'lib-fulfillment-modules'), path_1.default.join(workspaceRoot, 'packages', 'fulfillment', 'lib-fulfillment-modules'), path_1.default.join(workspaceRoot, 'packages', 'lib-fulfillment-modules'));
    }
    ...
    if (exactPath && !paths.includes(exactPath)) {
        ...
        console.log(`  📍 动态发现补充路径: ${packageName} -> ${path_1.default.relative(workspaceRoot, exactPath)}`);
    }
    ...
    for (const dir of commonDirs) {
        for (const pattern of namePatterns) {
            const candidatePath = path_1.default.join(workspaceRoot, dir, pattern);
 ...
 */
function findExactWorkspacePackagePath(workspaceRoot, packageName) {
    try {
        // 缓存包映射以提高性能
 ...
 * 构建真实的workspace包映射
 */
function buildRealWorkspacePackageMap(workspaceRoot) {
 ...
 */
function inferWorkspacePathFromPnpm(workspaceRoot, pnpmPath, packageName) {
    ...
    if (match) {
        ...
        // 重构路径
        const workspacePath = path_1.default.join(workspaceRoot, 'packages', ...pathComponents);
