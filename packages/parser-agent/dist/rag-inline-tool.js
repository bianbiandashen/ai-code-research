...
exports.RagInlineTool = void 0;
exports.searchCodeEntities = searchCodeEntities;
exports.getRelatedCodeEntities = getRelatedCodeEntities;
 ...
 */
class RagInlineTool {
    constructor(entitiesPath, projectRoot) {
        this.entities = []; // 所有加载的实体
        this.entityMap = new Map(); // 实体ID到实体的快速查找映射
        this.projectRoot = projectRoot || process.cwd();
        this.logger = new shared_utils_1.SimpleLogger(this.projectRoot);
        ...
        // 设置API密钥（如果环境变量中没有的话）
        process.env.XHS_AWS_BEDROCK_API_KEY = process.env.XHS_AWS_BEDROCK_API_KEY || 'aa74edef9cb44aab8a03f37f36197ec6';
    }
    /**
     * 从JSON文件加载实体数据
    ...
    loadEntities(entitiesPath) {
        try {
            // 获取文件状态
            const stats = fs_1.default.statSync(entitiesPath);
            const currentModifiedTime = stats.mtime.getTime();
            // 如果文件没有变化，则跳过加载
            if (this.lastModifiedTime && this.lastModifiedTime === currentModifiedTime) {
                return;
            }
            // 读取并解析JSON文件
            const data = fs_1.default.readFileSync(entitiesPath, 'utf8');
            ...
            this.logger.log(`已加载 ${this.entities.length} 个实体 (文件更新时间: ${new Date(currentModifiedTime).toLocaleString()})`);
        }
        catch (error) {
            this.logger.error(`加载实体数据失败: ${error.message}`);
            // 加载失败时重置数据
            this.entities = [];
            this.entityMap.clear();
        }
    ...
    setupFileWatcher(entitiesPath) {
        try {
            ...
            // 设置新的文件监听器
            this.fileWatcher = fs_1.default.watch(entitiesPath, (eventType, filename) => {
                if (eventType === 'change') {
            ...
            this.logger.warn(`设置文件监听失败: ${error.message}`);
        }
    }
    /**
     * 手动重新加载实体数据
    ...
    reloadEntities() {
        ...
        try {
            ...
            this.logger.error(`手动重新加载失败: ${error.message}`);
            return false;
        }
    }
    /**
    ...
    getFileStatus() {
        ...
        try {
            const stats = fs_1.default.statSync(this.entitiesFilePath);
            const accessible = fs_1.default.accessSync(this.entitiesFilePath, fs_1.default.constants.R_OK) === undefined;
            return {
                exists: true,
                readable: accessible,
            ...
            return { exists: false, readable: false, entityCount: this.entities.length };
        }
    }
    /**
     * 清理资源（关闭文件监听器）
     ...
     */
    async search(userQuery, topK = 5) {
        // 第一步：使用AI识别用户意图
        const intent = await this.identifyIntent(userQuery);
        // 第二步：计算每个实体与查询的相关性得分
        const relevanceScores = await this.calculateRelevanceScores(userQuery, intent);
        // 第三步：根据得分排序并选择前K个实体
     ...
     */
    async getRelatedEntities(entityId, maxRelated = 3) {
        // 查找源实体
        ...
        this.logger.log(`开始获取实体 ${entityId} 的相关实体，最大数量: ${maxRelated}, 实体为: ${JSON.stringify(sourceEntity)}`);
        // 初始化各种关系的实体集合
        ...
        if (sourceEntity.TEMPLATE_COMPONENTS) {
            for (const componentName of sourceEntity.TEMPLATE_COMPONENTS) {
                // 查找名称匹配的组件实体
                const matchingEntities = this.entities.filter(e => e.type === 'component' &&
                    (e.rawName === componentName || e.id.endsWith(`:${componentName}`)));
                for (const entity of matchingEntities) {
                    // 确保不重复添加已存在的实体
                    if (!this.isEntityInLists(entity, [importEntities, callEntities, templateEntities])) {
        ...
        const tagMatches = this.entities
            ...
            !this.isEntityInLists(entity, [importEntities, callEntities, templateEntities]) &&
        ...
        this.logger.log(`获取相关实体成功: 导入${importEntities.length}个, 调用${callEntities.length}个, 模板${templateEntities.length}个, 相似${similarEntities.length}个`);
        ...
        const prompt = this.buildRelatedPrompt(sourceEntity, allRelated, relationships);
        return {
            sourceEntity,
            relatedEntities: {
...
例如：选择器,地址管理,表单组件`;
            ...
            this.logger.error(`意图识别失败: ${error.message}`);
            // AI失败时返回原始查询
            return query;
        }
    }
     ...
     */
    async calculateRelevanceScores(query, intent) {
        ...
        try {
            ...
            if (totalEntities <= 50) {
                ...
                strategy = '大规模分片评分';
                finalScores = await this.multiStageScoring(query, intent, preFilterResults);
            }
            // 生成最终统计
            ...
            this.logger.error(`相关性评分失败 (耗时${errorTime}ms): ${error.message}`);
    ...
    quickTextFilter(query, intent) {
        ...
        this.entities.forEach(entity => {
            ...
            // 计算关键词匹配度
            const matches = allSearchWords.filter(word => entityText.includes(word));
     ...
     * 获取实体类型相关性加成
     */
    getTypeRelevanceBonus(entityType, query) {
     ...
     * 小规模：直接AI评分
     */
    async directAIScoring(query, intent, entities) {
        return await this.batchAIScoring(query, intent, entities, '全量AI评分');
    }
    /**
     * 中等规模：混合评分策略
     */
    async hybridScoring(query, intent, preFilterResults) {
        const startTime = Date.now();
        // 选择预筛选得分较高的实体进行AI评分
        ...
        const aiScores = await this.batchAIScoring(query, intent, candidates, '混合评分');
        // 合并预筛选结果和AI评分结果
     ...
     * 大规模：多阶段评分策略
     */
    async multiStageScoring(query, intent, preFilterResults) {
        const startTime = Date.now();
        ...
        const batchPromises = batches.map(async (batch, index) => {
            ...
            const batchScores = await this.batchAIScoring(query, intent, batch, `批次${index + 1}`);
            const batchTime = Date.now() - batchStart;
            this.logger.log(`✅ 批次${index + 1}完成: 耗时${batchTime}ms 评分${Object.keys(batchScores).length}个实体`);
        ...
        this.logger.log(`🎯 并行批次评分完成: 总耗时${totalBatchTime}ms 成功${successfulBatches}/${batches.length}批次 失败${failedBatches}批次 总评分${totalScoredEntities}个实体`);
        // 检查是否所有批次都失败了
        ...
        if (highScoreEntities.length > 0) {
            const precisionScores = await this.precisionAIScoring(query, intent, highScoreEntities);
     ...
     * 5. 组件的注释信息是否与查询相关
     */
    async batchAIScoring(query, intent, entities, stage) {
        const startTime = Date.now();
...
只返回JSON格式：{"组件ID": 分数}`;
        ...
        const entityInfo = entities.map((entity) => {
            ...
            const annotation = entity.ANNOTATION ? entity.ANNOTATION.substring(0, 50) : '';
            return `
ID: ${entity.id}
类型: ${entity.type} 
摘要: ${summary} 
...
评分:`;
        ...
        try {
            const { text } = await (0, ai_1.generateText)({
                // @ts-ignore
                model: this.anthropic("claude-3-7-sonnet-latest", { structuredOutputs: true }),
                system,
                prompt,
                temperature: 0.1,
            });
            ...
            const scores = JSON.parse((0, jsonrepair_1.jsonrepair)(jsonMatch[0]));
            const scoredCount = Object.keys(scores).length;
            const scoreValues = Object.values(scores).map(s => Number(s));
            const avgScore = scoreValues.reduce((a, b) => a + b, 0) / scoredCount;
            this.logger.log(`✅ ${stage}成功: 耗时${aiTime}ms 评分${scoredCount}/${entities.length}个实体 平均分${avgScore.toFixed(1)}`);
            return scores;
        }
        catch (error) {
            const errorTime = Date.now() - startTime;
     ...
     * 精确AI评分（用于高分实体的二次评分）
     */
    async precisionAIScoring(query, intent, entities) {
        const startTime = Date.now();
...
精确评分:`;
        ...
        try {
            ...
            this.logger.log(`✨ 精确评分完成: 耗时${precisionTime}ms 评分${scoredCount}个实体 平均分${avgScore.toFixed(1)} 最高分${maxScore}`);
            return scores;
        }
        catch (error) {
            const errorTime = Date.now() - startTime;
     ...
     * @returns 实体ID到相关性得分的映射
     */
    calculateFallbackRelevanceScores(query, intent) {
        const scores = {};
        const queryWords = this.extractKeywords(query.toLowerCase());
        const intentWords = this.extractKeywords(intent.toLowerCase());
        const allSearchWords = [...queryWords, ...intentWords];
        this.entities.forEach(entity => {
            ...
            if (entity.summary) {
                ...
                const summaryMatches = allSearchWords.filter(word => summaryWords.some(summaryWord => summaryWord.includes(word) || word.includes(summaryWord)));
                score += (summaryMatches.length / allSearchWords.length) * 4.0;
            }
            // 2. 标签匹配 (权重: 30%)
            if (entity.tags && entity.tags.length > 0) {
                const tagWords = entity.tags.map(tag => tag.toLowerCase()).join(' ');
                const tagMatches = allSearchWords.filter(word => tagWords.includes(word));
            ...
            const idTypeMatches = allSearchWords.filter(word => idTypeWords.some(idWord => idWord.includes(word) || word.includes(idWord)));
            score += (idTypeMatches.length / allSearchWords.length) * 2.0;
            // 4. 注释匹配 (权重: 10%)
            if (entity.ANNOTATION) {
                const annotationWords = this.extractKeywords(entity.ANNOTATION.toLowerCase());
                const annotationMatches = allSearchWords.filter(word => annotationWords.some(annWord => annWord.includes(word) || word.includes(annWord)));
        ...
        if (maxScore < 1) {
            this.entities.forEach(entity => {
                // 对常见组件类型给予基础分数
                if (entity.type.includes('component') || entity.type.includes('page') || entity.type.includes('api')) {
    ...
    extractKeywords(text) {
        const stopWords = new Set(['the', 'a', 'an', 'and', 'or', 'but', 'in', 'on', 'at', 'to', 'for', 'of', 'with', 'by', 'is', 'are', 'was', 'were', 'be', 'been', 'have', 'has', 'had', '的', '是', '在', '和', '或者', '但是', '对于', '关于', '使用', '通过']);
        return text
            .split(/\s+|[,，。.!！?？;；:：]/)
     ...
     * @returns 构建的提示词
     */
    buildPrompt(query, entities, relevanceScores) {
        let prompt = `用户查询: "${query}"\n\n根据查询，找到了以下相关的代码组件：\n\n`;
        entities.forEach((entity, index) => {
            const score = relevanceScores[entity.id] || 0;
            prompt += `### 组件 ${index + 1}: ${entity.id} (相关度: ${score})\n`;
            // 使用绝对路径便于定位文件
            const absoluteFilePath = path_1.default.join(this.projectRoot, entity.file);
            ...
            // 尝试读取并添加实际代码内容
            try {
                const filePath = path_1.default.join(this.projectRoot, entity.file);
   ...
   * @returns 构建的关系分析提示词
   */
    buildRelatedPrompt(sourceEntity, relatedEntities, relationships) {
        let prompt = `## 源组件: ${sourceEntity.id}\n`;
        // 添加源组件的基本信息
        const absoluteFilePath = path_1.default.join(this.projectRoot, sourceEntity.file);
        prompt += `文件路径: ${absoluteFilePath}\n`;
        ...
        if (sourceEntity.TEMPLATE_COMPONENTS && sourceEntity.TEMPLATE_COMPONENTS.length > 0) {
        ...
        relatedEntities.forEach((entity, index) => {
            ...
            const relationDescription = relationTypes.map(type => {
                switch (type) {
                    case 'IMPORTS': return '被源组件导入';
                    case 'CALLS': return '被源组件调用';
                    case 'TEMPLATE': return '在源组件模板中使用';
                    case 'SIMILAR_TAG': return '具有相似标签';
                    default: return type;
                }
            }).join(', ');
            prompt += `### 相关组件 ${index + 1}: ${entity.id} (${relationDescription})\n`;
            const entityAbsolutePath = path_1.default.join(this.projectRoot, entity.file);
            ...
            try {
                ...
                if (fs_1.default.existsSync(filePath)) {
                    const code = fs_1.default.readFileSync(filePath, 'utf8');
                    // 限制代码长度避免提示词过长
                    const truncatedCode = code.length > 1500 ? code.substring(0, 1500) + '\n...(代码已截断)' : code;
                    prompt += `\n代码片段:\n\`\`\`\n${truncatedCode}\n\`\`\`\n`;
                }
            }
            catch (error) {
        ...
        Object.entries(relationStats).forEach(([relationType, entityIds]) => {
            ...
            prompt += `- **${typeDescription}** (${entityIds.length}个): ${entityIds.join(', ')}\n`;
        });
     ...
     */
    addRelationship(relationships, entityId, relationType) {
     ...
     */
    isEntityInLists(entity, entityLists) {
     ...
     */
    async getSmartRelatedEntities(entityId, userRequirement, maxEntities = 3) {
        // 查找源实体
        const sourceEntity = this.entityMap.get(entityId);
        if (!sourceEntity) {
            this.logger.error(`未找到ID为 ${entityId} 的实体`);
        ...
        // 使用AI进行智能筛选
        const { smartSelection, reasoning } = await this.selectSmartEntities(sourceEntity, candidateEntities, userRequirement, maxEntities);
        return {
            sourceEntity,
            smartSelection,
            reasoning,
     ...
     * @returns 选择结果和推理过程
     */
    async selectSmartEntities(sourceEntity, candidates, userRequirement, maxEntities) {
        try {
            // 根据 maxEntities 调整描述文案
            const selectInstruction = maxEntities === 1 ?
                '选择与用户需求最相关的 1 个实体' :
...
${entity.TEMPLATE_COMPONENTS ? `- 使用组件: ${entity.TEMPLATE_COMPONENTS.join(', ')}` : ''}
`).join('\n')}
...
请从以上候选实体中${selectInstruction}。`;
            ...
            this.logger.error(`智能实体选择失败: ${error.message}`);
            return {
                smartSelection: candidates.slice(0, maxEntities),
                reasoning: `AI分析出错: ${error.message}，使用默认选择`
            };
        }
    }
}
exports.RagInlineTool = RagInlineTool;
// 简便的函数接口，用于快速调用搜索功能
async function searchCodeEntities(query, entitiesPath, projectRoot, topK = 5) {
    const tool = new RagInlineTool(entitiesPath, projectRoot);
    return await tool.search(query, topK);
}
// 简便的函数接口，用于快速调用关系查询功能
async function getRelatedCodeEntities(entityId, entitiesPath, projectRoot, maxRelated = 3) {
    const tool = new RagInlineTool(entitiesPath, projectRoot);
