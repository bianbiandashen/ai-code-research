...
exports.EnrichedDataAnalyzer = void 0;
...
class EnrichedDataAnalyzer {
    constructor(projectPath) {
        ...
        this.enrichedDataPath = path_1.default.join(projectPath, 'data', 'entities.enriched.json');
    }
    /**
     * 加载 enriched 数据
     */
    async loadEnrichedData() {
        try {
            if (fs_1.default.existsSync(this.enrichedDataPath)) {
                const data = fs_1.default.readFileSync(this.enrichedDataPath, 'utf-8');
     ...
     * 按目录分组实体
     */
    groupEntitiesByDirectory() {
        const directoryMap = new Map();
        this.enrichedEntities.forEach(entity => {
            const dirPath = path_1.default.dirname(entity.file);
     ...
     * 生成目录语义描述
     */
    generateDirectorySemanticDescription(entities) {
     ...
     * 生成增强的目录用途描述
     */
    generateEnrichedPurpose(entities) {
     ...
     * 检测目录特殊工程含义
     */
    detectDirectorySpecialMeaning(dirPath, entities) {
     ...
     * 计算文件重要性
     */
    calculateFileImportance(entity) {
     ...
     * 获取文件特殊含义
     */
    getFileSpecialMeaning(entity) {
        const fileName = path_1.default.basename(entity.file);
        // 基于文件名模式
    ...
    identifyEngineeringPatterns() {
        ...
        // 分析服务模式
        const serviceEntities = this.enrichedEntities.filter(e => e.tags?.includes('API接口') || e.tags?.includes('服务层'));
     ...
     * 分析文件夹规范
     */
    analyzeFolderStandards() {
        const recommendedStandards = [];
        const userCustomStandards = [];
     ...
     * 生成推荐标准
     */
    generateRecommendedStandard(dirPath, info) {
        const dirName = path_1.default.basename(dirPath);
     ...
     * 计算合规性得分
     */
    calculateComplianceScore(standards) {
        ...
        standards.forEach(standard => {
            ...
            const matchingFiles = files.filter(file => expectedTypes.some(type => file.endsWith(type)));
    ...
    getDirectoryFiles(dirPath) {
        return this.enrichedEntities
            .filter(entity => path_1.default.dirname(entity.file) === dirPath)
            .map(entity => entity.file);
    }
    /**
     * 获取主要文件类型
     */
    getMainFileTypes(entities) {
        const typeCount = new Map();
        entities.forEach(entity => {
            const ext = path_1.default.extname(entity.file);
     ...
     * 计算目录重要性
     */
    calculateDirectoryImportance(dirPath, entities) {
        const dirName = path_1.default.basename(dirPath);
        let importance = 5;
        // 基于目录名称
        ...
        if (entities.length > 20)
            importance += 1;
        // 基于内容重要性
        const hasHighValueContent = entities.some(e => e.tags?.some(tag => ['应用入口', '路由配置', '状态管理'].includes(tag)));
        if (hasHighValueContent)
            importance += 2;
        return Math.min(importance, 10);
    }
    /**
     ...
     * 生成建议
     */
    generateRecommendations(criticalDirectories, criticalFiles) {
...
exports.EnrichedDataAnalyzer = EnrichedDataAnalyzer;
