...
exports.BusinessComponentAnalyzer = void 0;
class BusinessComponentAnalyzer {
    ...
    identifyComponents() {
        const components = [];
        const componentUsage = new Map(); // 组件名 -> 使用它的文件列表
        // 首先识别所有可能的组件
        const componentEntities = this.entities.filter(entity => this.isComponent(entity.file) || this.isComponentEntity(entity));
    ...
    isLocalComponentImport(importPath) {
    ...
    extractComponentNameFromEntity(entity) {
     ...
     * 查找组件的消费者（多种匹配方式）
     */
    findComponentConsumers(componentName, componentUsage, componentFile) {
        const consumers = [];
        // 1. 直接名称匹配
     ...
     * 获取组件名称的不同变体
     */
    getComponentNameVariations(componentName) {
        const variations = [];
    ...
    generateComponentDescription(entity, componentName) {
    ...
    analyzeFunctionality(entity) {
        ...
        if (entity.CALLS && Array.isArray(entity.CALLS)) {
            const apiCalls = entity.CALLS.filter(call => call.includes('api') || call.includes('request') || call.includes('fetch'));
    ...
    generateComponentStats(components) {
    ...
    generateDocumentation(components) {
        ...
        Object.entries(componentsByType).forEach(([type, typeComponents]) => {
            if (typeComponents.length > 0) {
                doc += `### ${typeNames[type]} (${typeComponents.length}个)\n\n`;
...
exports.BusinessComponentAnalyzer = BusinessComponentAnalyzer;
