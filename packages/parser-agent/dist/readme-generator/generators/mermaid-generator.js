...
class MermaidGenerator {
     ...
     * 生成所有相关的Mermaid图表
     */
    generateAllDiagrams(analysis, entities, architectureInsights) {
        const diagrams = [];
        // 1. 项目架构依赖图
     ...
     * 生成项目架构依赖图
     */
    generateArchitectureDiagram(analysis, entities) {
     ...
     * 生成DDD数据流图
     */
    generateDDDDataFlowDiagram(dddAnalysis) {
     ...
     * 生成组件依赖图
     */
    generateComponentDependencyDiagram(entities) {
        const components = entities.filter(e => e.type === 'component' || e.file.endsWith('.vue') || e.file.endsWith('.tsx') || e.file.endsWith('.jsx')).slice(0, 12); // 限制数量
        let content = 'flowchart TD\n';
        // 为组件生成更好的显示名称和ID
     ...
     * 生成分层架构图
     */
    generateLayerDiagram(layerStructure) {
        let content = 'flowchart TD\n';
        // 表现层
        if (layerStructure.presentation.length > 0) {
            content += '    subgraph PRES["🖥️ 表现层 (Presentation)"]\n';
            layerStructure.presentation.slice(0, 5).forEach((item, index) => {
        ...
        if (layerStructure.business.length > 0) {
            ...
            layerStructure.business.slice(0, 5).forEach((item, index) => {
        ...
        if (layerStructure.data.length > 0) {
            ...
            layerStructure.data.slice(0, 5).forEach((item, index) => {
        ...
        if (layerStructure.infrastructure.length > 0) {
            ...
            layerStructure.infrastructure.slice(0, 5).forEach((item, index) => {
     ...
     * 生成技术栈分布饼图
     */
    generateTechStackPieChart(analysis) {
     ...
     * 添加常见的架构连接关系
     */
    addCommonArchitectureConnections(content, directories) {
     ...
     * 分析目录间的依赖关系
     */
    analyzeDependenciesBetweenDirectories(directories, entities) {
        const dependencies = [];
     ...
     * 查找导入文件所属的目录
     */
    findImportDirectory(importPath, entities, dirPaths) {
        // 直接路径匹配
     ...
     * 获取组件的显示名称
     */
    getComponentDisplayName(entity) {
     ...
     * 获取组件图标
     */
    getComponentIcon(compType, displayName) {
        const lowerName = displayName.toLowerCase();
        if (lowerName.includes('page') || lowerName.includes('dashboard'))
            return '📄';
     ...
     * 根据组件名查找组件
     */
    findComponentByName(componentName, componentMap) {
        // 遍历所有组件，查找匹配的
     ...
     * 根据导入路径查找组件
     */
    findComponentByImportPath(importPath, componentMap) {
        // 清理导入路径
     ...
     * 添加推测的组件连接关系
     */
    addInferredComponentConnections(content, componentMap) {
        const components = Array.from(componentMap.values());
        // 页面组件连接到其他组件
        const pages = components.filter(c => c.file.includes('/page') || c.displayName.toLowerCase().includes('page') ||
            c.file.includes('/dashboard') || c.displayName.toLowerCase().includes('dashboard'));
        const forms = components.filter(c => c.displayName.toLowerCase().includes('form'));
        const modals = components.filter(c => c.displayName.toLowerCase().includes('modal'));
        const status = components.filter(c => c.displayName.toLowerCase().includes('status'));
        // 页面 -> 表单
        pages.forEach(page => {
            forms.forEach(form => {
