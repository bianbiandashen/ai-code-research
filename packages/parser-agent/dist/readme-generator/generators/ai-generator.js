...
exports.ReadmeContentGenerator = exports.AIDocumentGenerator = void 0;
...
const route_config_analyzer_1 = require("../analyzers/route-config-analyzer");
...
class AIDocumentGenerator {
     ...
     */
    constructor(aiService = ai_service_1.defaultAIService) {
     ...
     */
    filterImportantDirectories(directories, isRoute) {
        ...
        const filtered = directories.filter((dir) => {
            const dirName = path_1.default.basename(dir.path);
            const dirPath = dir.path.toLowerCase();
            // Exclude specific patterns
            return !excludePatterns.some(pattern => dirName.startsWith(pattern) ||
     ...
     */
    async generateProjectArchitectureOverview(analysis, entities, customContent) {
        // 过滤掉workspace实体
        const filteredEntities = entities.filter(entity => entity.isWorkspace === false);
        ...
        const prompt = prompts_1.PromptTemplates.generateArchitectureOverviewPrompt(promptData);
        try {
     ...
     */
    analyzeCoreTypes(entities) {
        const typeFiles = entities.filter(entity => entity.file.includes('types') ||
     ...
     */
    async generateDetailedDirectoryCodeAnalysis(analysis, entities, customContent, isRoute) {
        // 过滤掉workspace实体
        const filteredEntities = entities.filter(entity => entity.isWorkspace === false);
        const directoryMap = this.groupEntitiesByDirectory(filteredEntities);
        const importantDirectories = this.filterImportantDirectories(analysis.structure.directories, isRoute);
        ...
        for (let i = 0; i < Math.min(importantDirectories.length, 20); i += batchSize) { // 增加到20个目录
            ...
            const batchAnalysis = await this.processBatchDirectories(batch, directoryMap, analysis, customContent, isRoute);
     ...
     */
    async processBatchDirectories(directories, directoryMap, analysis, customContent, isRoute) {
        const dir = directories[0];
        const allFolderEntities = directoryMap.get(dir.path) || [];
        if (allFolderEntities.length === 0) {
            return formatters_1.TemplateFormatters.generateEmptyDirectoryAnalysis(dir);
        }
        // Group entities by file, take entities from top 4 files
        const entitiesByFile = new Map();
        allFolderEntities.forEach(entity => {
            if (isRoute) {
                const isRoutePath = directories.find(directory => directory.fullPath === entity.file);
                if (!isRoutePath)
                    return;
                dir.fullPath = isRoutePath.fullPath;
                dir.purpose = '页面路由入口文件';
                if (!entitiesByFile.has(isRoutePath.fullPath)) {
                    entitiesByFile.set(isRoutePath.fullPath, []);
                }
                entitiesByFile.get(isRoutePath.fullPath).push({ ...entity, file: isRoutePath.fullPath });
            }
            else {
        ...
        if (representativeEntities.length === 0) {
            return formatters_1.TemplateFormatters.generateInvalidEntitiesDirectoryAnalysis(dir);
        }
        // 构建目录分析 Prompt 数据
        const promptData = {
            dir,
        ...
        const prompt = prompts_1.PromptTemplates.generateDirectoryAnalysisPrompt(promptData, isRoute);
        try {
     ...
     */
    generateFallbackDirectoryAnalysis(dir, entities) {
        return formatters_1.TemplateFormatters.generateFallbackDirectoryAnalysis(dir, entities);
    }
    /**
     * 生成回退架构概览
     *
     ...
     */
    generateFallbackArchitectureOverview(analysis, entities) {
        return formatters_1.TemplateFormatters.generateFallbackArchitectureOverview(analysis, entities);
    }
    /**
     * 生成所有工程洞察
     *
     ...
     */
    async generateAllInsights(analysis, projectPath, entities, customContent) {
        ...
        // 初始化分析器
        this.architectureAnalyzer = new architecture_analyzer_1.ArchitectureAnalyzer();
        this.mermaidGenerator = new mermaid_generator_1.MermaidGenerator();
        this.routeAnalyzer = new route_config_analyzer_1.RouteConfigAnalyzer(projectPath, [], this.aiService, customContent);
        // 1. 生成包含自定义内容的架构概览
        ...
        try {
            ...
            // 5. 整合所有分析结果
            const comprehensiveAnalysis = await this.buildComprehensiveAnalysis(architectureOverview, detailedCodeAnalysis, mermaidDiagrams, analysis, filteredEntities, customContent);
            return {
     ...
     */
    async buildComprehensiveAnalysis(architectureOverview, detailedCodeAnalysis, mermaidDiagrams, analysis, entities, customContent) {
        let report = '';
        ...
        if (mermaidDiagrams && Array.isArray(mermaidDiagrams) && mermaidDiagrams.length > 0) {
            ...
            mermaidDiagrams.filter((diagram) => !!diagram.title).slice(0, 3).forEach((diagram) => {
     ...
     */
    generateFallbackInsights(analysis, entities) {
     ...
     */
    generateSimpleCodeAnalysis(analysis, entities) {
        // 过滤掉workspace实体
        const filteredEntities = entities.filter(entity => entity.isWorkspace === false);
        const directoryMap = this.groupEntitiesByDirectory(filteredEntities);
        const importantDirectories = this.filterImportantDirectories(analysis.structure.directories);
        return formatters_1.TemplateFormatters.generateSimpleCodeAnalysis(analysis, filteredEntities, directoryMap, importantDirectories);
    }
    /**
     * 格式化增强路由分析结果
     *
     ...
     */
    formatEnhancedRouteAnalysis(enhancedResult) {
        return formatters_1.TemplateFormatters.formatEnhancedRouteAnalysis(enhancedResult);
    }
    /**
     * 生成路由配置分析
     */
    async generateRouteConfigAnalysis(analysis, filteredEntities) {
        if (!this.routeAnalyzer)
            return "";
        try {
            console.log("🌐 开始路由配置分析...");
            const enrichedEntities = filteredEntities;
     ...
     */
    formatDataFlowAnalysis(dataFlowResult) {
        return formatters_1.TemplateFormatters.formatDataFlowAnalysis(dataFlowResult);
    }
}
exports.AIDocumentGenerator = AIDocumentGenerator;
/**
 * README内容生成器
 *
...
class ReadmeContentGenerator {
     ...
     */
    async generateContent(analysis, template, language, projectPath, customFile) {
        ...
        let customContent = '';
        if (customFile && fs_1.default.existsSync(customFile)) {
            try {
                customContent = fs_1.default.readFileSync(customFile, 'utf8');
     ...
     */
    buildReadme(aiGenerator, analysis, insights, entities, customContent) {
        return formatters_1.TemplateFormatters.buildReadme(aiGenerator, analysis, insights, entities, customContent);
    }
    /**
     * 生成回退README
     *
     ...
     */
    generateFallbackReadme(analysis) {
        return formatters_1.TemplateFormatters.generateFallbackReadme(analysis);
    }
}
exports.ReadmeContentGenerator = ReadmeContentGenerator;
