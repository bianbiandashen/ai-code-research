...
const readme_generator_1 = require("./readme-generator");
 ...
 */
function displayExtractionStats(entities, startTime) {
    ...
    const fileStats = {};
    entities.forEach(entity => {
        const ext = path_1.default.extname(entity.file).toLowerCase() || 'no-ext';
 ...
 */
function parseReadmeStructure(content) {
 ...
 * 合并新内容和现有用户内容
 */
function mergeReadmeContent(newContent, userSections, preservedContent) {
 ...
 * @param options 命令选项
 */
async function executePatchReadme(rootDir, options) {
    ...
    try {
        ...
        if (!options.output || options.output.trim() === '') {
            ...
            outputPath = path_1.default.join(rootDir, filename);
            console.log(`📝 使用默认输出路径，生成文件名: ${filename}`);
        }
        else {
            outputPath = path_1.default.isAbsolute(options.output)
                ? options.output
                : path_1.default.join(rootDir, options.output);
            // 检查是否是目录或需要生成时间戳文件名
            if (fs_1.default.existsSync(outputPath) && fs_1.default.statSync(outputPath).isDirectory()) {
                ...
                // 检查父目录是否存在且是目录
                const parentDir = path_1.default.dirname(outputPath);
                if (fs_1.default.existsSync(parentDir) && fs_1.default.statSync(parentDir).isDirectory()) {
                    const basename = path_1.default.basename(options.output);
                    if (!basename || basename === '.md' || basename === 'readme.md') {
                        // 生成带时间戳的文件名
                        const now = new Date();
                        const timestamp = `${now.getFullYear()}${String(now.getMonth() + 1).padStart(2, '0')}${String(now.getDate()).padStart(2, '0')}${String(now.getHours()).padStart(2, '0')}${String(now.getMinutes()).padStart(2, '0')}${String(now.getSeconds()).padStart(2, '0')}`;
                        const filename = `readme-${timestamp}.md`;
                        outputPath = path_1.default.join(parentDir, filename);
                        console.log(`📝 检测到目录路径，自动生成文件名: ${filename}`);
                    }
                }
            }
        ...
        const newContent = await (0, readme_generator_1.generateReadme)(rootDir, 'engineering-focused', // 使用工程聚焦的模板
        options.language, options.custom);
        // 如果有现有README，进行智能合并
 ...
 * @param options 命令选项
 */
async function executeGenerateReadme(rootDir, options) {
    const startTime = Date.now();
    try {
        ...
        if (!options.preview && !options.force && fs_1.default.existsSync(outputPath)) {
        ...
        if (options.preview) {
            ...
            const readmeContent = await (0, readme_generator_1.generateReadme)(rootDir, options.template, options.language, options.custom);
        ...
        await (0, readme_generator_1.generateReadmeToFile)(rootDir, outputPath, options.template, options.language, options.custom);
        // 显示生成结果统计
        const endTime = Date.now();
        const duration = endTime - startTime;
        const fileStats = fs_1.default.statSync(outputPath);
        ...
        console.error('❌ README 生成失败:', error.message);
        if (process.env.DEBUG || options.verbose) {
            console.error('详细错误信息:', error);
        }
        console.error('\n💡 常见问题解决方案:');
...
program
    ...
    .option('-o, --out <file>', '输出JSON文件路径', path_1.default.join(process.cwd(), 'data/entities.json'))
    ...
    try {
        ...
        // 提取实体
        const entities = await (0, fileWalker_1.extractAllEntities)(rootDir);
        ...
        if (!options.dryRun) {
            try {
                // 确保输出目录存在
                const outputDir = path_1.default.dirname(options.out);
                ensureDirectoryExists(outputDir);
                // 写入文件
                fs_1.default.writeFileSync(options.out, jsonOutput, 'utf8');
                // 获取文件大小信息
                const fileStats = fs_1.default.statSync(options.out);
                const fileSize = formatFileSize(fileStats.size);
                console.log(`\n💾 结果已保存到: ${options.out}`);
                console.log(`📁 文件大小: ${fileSize}`);
            }
...
program
    ...
    try {
        ...
        try {
            ...
            console.error('❌ JSON格式错误:', parseError.message);
            process.exit(1);
        }
        // 验证数据结构
...
program
    ...
    if (!process.argv.slice(2).some(arg => ['extract', 'validate'].includes(arg))) {
        ...
        try {
            ...
            ensureDirectoryExists(outputDir);
            // 保存到文件
            fs_1.default.writeFileSync(options.out, JSON.stringify(entities, null, 2));
            console.log(`\n✅ 已保存到: ${options.out}`);
        }
        catch (error) {
