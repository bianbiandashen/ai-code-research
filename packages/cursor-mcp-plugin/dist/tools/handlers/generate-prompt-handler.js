...
class GeneratePromptHandler extends base_handler_1.BaseHandler {
    constructor(hasEntitiesFile) {
        super(hasEntitiesFile);
    }
    async handle(sessionId, additionalContext, workspaceEntities) {
        try {
                    ...
                    for (const entityId of entityIds) {
                        ...
                        if (conversation.workspaceResults) {
                            ...
                            if (wsResult && wsResult.entities) {
                                foundEntity = wsResult.entities.find((e) => e.id === entityId);
                            }
                        }
                        ...
                        if (!foundEntity && conversation.workspaceRelatedEntities) {
                            ...
                            if (wsRelatedEntities) {
                                ...
                                foundEntity = allRelated.find((e) => e.id === entityId);
                            }
                        }
                        ...
                        if (!foundEntity && conversation.workspaceDefaultSelection) {
                            ...
                            if (wsDefaultEntities) {
                                foundEntity = wsDefaultEntities.find((e) => e.id === entityId);
                            }
                        }
                        if (foundEntity) {
            ...
            const entitiesWithAbsolutePaths = (0, entity_utils_1.convertEntitiesToAbsolutePaths)(entities);
            // 生成最终的代码提示词（使用转换后的实体）
            const finalPrompt = (0, prompt_generator_1.generateCodePrompt)(userInput, entitiesWithAbsolutePaths, contextInfo.trim() || undefined);
            // 更新会话状态为完成
