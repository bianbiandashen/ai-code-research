...
export class QuickAnalysisHandler extends BaseHandler {
  async handle(input: string, componentIndex: number = 0, includeRelated: boolean = true, maxRelated: number = 1, additionalContext?: string): Promise<ToolResult> {
...
3. 检查项目代码库是否包含相关内容`
      ...
      const allWorkspaceEntities: any[] = [];
      const allContextInfo: string[] = [];
      ...
      for (const [workspacePath, wsResult] of searchResult.workspaceResults) {
        ...
        const sortedEntities = wsResult.entities
          .sort((a: any, b: any) => (wsResult.relevanceScores[b.id] || 0) - (wsResult.relevanceScores[a.id] || 0));
        
        if (sortedEntities.length === 0) continue;
        ...
        const coreEntity = enrichedToEntity(coreComponent);
        ...
        if (includeRelated) {
          try {
            ...
            if (smartResult && smartResult.smartSelection.length > 0) {
                             const relatedEntities = smartResult.smartSelection.map((entity: any) => {
      ...
      const finalPrompt = await generateCodePrompt(input, allWorkspaceEntities, contextInfo.trim() || undefined);
...
${Array.from(searchResult.workspaceResults.entries()).map(([workspacePath, _]) => {
  const workspaceName = require('path').basename(workspacePath);
  const workspaceEntities = allWorkspaceEntities.filter((entity: any) => entity.workspacePath === workspacePath);
  return `### 📂 ${workspaceName}
${workspaceEntities.map((entity: any, i: number) => 
  `${i + 1}. **${entity.id}** (${entity.type})
   - 📁 文件: ${entity.file}
