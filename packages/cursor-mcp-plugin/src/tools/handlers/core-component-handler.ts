...
export class CoreComponentHandler extends BaseHandler {
  ...
  private async handleWorkspaceEntities(sessionId: string, workspaceEntityMap: Map<string, string[]>, maxRelated: number, conversation: any, originalWorkspaceEntities: string): Promise<ToolResult> {
    ...
    for (const [workspacePath, components] of workspaceComponentMap) {
      ...
      let workspaceAiReasoningParts: string[] = [];
      ...
      for (const component of components) {
        ...
        if (smartResult) {
          const relatedEntities = smartResult.smartSelection.map(enrichedToEntity);
          ...
          // 合并候选实体
          workspaceLimitedRelatedEntities.imports.push(...smartResult.allCandidates.imports.map(enrichedToEntity));
          workspaceLimitedRelatedEntities.calls.push(...smartResult.allCandidates.calls.map(enrichedToEntity));
          workspaceLimitedRelatedEntities.templates.push(...smartResult.allCandidates.templates.map(enrichedToEntity));
          workspaceLimitedRelatedEntities.similar.push(...smartResult.allCandidates.similar.map(enrichedToEntity));
        }
      }
...
`;
   ...
   * 格式化下一步操作选项
   */
  private formatNextStepsActions(sessionId: string, workspaceEntities: string, workspaceDefaultSelectionMap: Map<string, Entity[]>): string {
...
≋≋≋ CLINE_ACTIONS_JSON_END ≋≋≋`;
  ...
  async handle(sessionId: string, maxRelated: number = 1, workspaceEntities?: string): Promise<ToolResult> {
    try {
