...
import { buildExampleSelections } from "../../utils/workspace-entity-utils";
...
export class StartAnalysisHandler extends BaseHandler {
   ...
   */
  private formatNextStepsActions(sessionId: string, workspaceResults: Map<string, any>, input: string): string {
          if (!serverConfig.returnDirect) {
        // 构建示例实体选择（展示跨workspace能力）
        const exampleSelections = buildExampleSelections(workspaceResults);
...
- 复制上面任意一个代码块在 Cursor 中执行`;
    ...
    const actions = {
      ...
      workspaceResults: Array.from(workspaceResults.entries()).map(([workspacePath, wsResult]) => ({
        ...
        entities: wsResult.entities.map((entity: any) => ({
          id: entity.id,
          type: entity.type,
          file: entity.file,
