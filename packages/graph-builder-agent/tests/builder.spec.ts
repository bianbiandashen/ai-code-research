...
// import { buildGraph } from "../src/builder";
// import { FileEntity, EnrichedEntity } from "../src/types";
...
//   REL_CALLS,
// } from "../src/schema";
// import { CodeRAG } from "../src/rag";
// import "dotenv/config";

// describe("Builder - 真实数据库测试", () => {
...
//       for (let i = 0; i < CREATE_SCHEMA_STATEMENTS.length; i++) {
//         const statement = CREATE_SCHEMA_STATEMENTS[i];
//         try {
...
//           if (
//             statement.includes("CREATE SPACE") ||
...
//     console.log("🔍 验证文件节点...");
//     try {
//       const fileQuery = `FETCH PROP ON ${FILE_TAG} "${files[0].id}" YIELD vertex AS v`;
...
//     console.log("🔍 验证实体节点...");
//     try {
//       const entityQuery = `FETCH PROP ON ${ENTITY_TAG} "${entities[0].id}" YIELD vertex AS v`;
//       const entityResult = await nebulaClient.executeNgql(entityQuery);
//       expect(entityResult.data).toBeDefined();
...
//     console.log("🔍 验证包含关系...");
//     try {
//       const containsQuery = `GO FROM "${files[0].id}" OVER ${REL_CONTAINS} YIELD edge AS contains_edge`;
//       const containsResult = await nebulaClient.executeNgql(containsQuery);
//       expect(containsResult.data).toBeDefined();
...
//     console.log("🔍 验证导入关系...");
//     try {
//       const importsQuery = `GO FROM "${entities[0].id}" OVER ${REL_IMPORTS} YIELD edge AS imports_edge`;
//       const importsResult = await nebulaClient.executeNgql(importsQuery);
//       expect(importsResult.data).toBeDefined();
...
//     console.log("🔍 验证调用关系...");
//     try {
//       const callsQuery = `GO FROM "${entities[0].id}" OVER ${REL_CALLS} YIELD edge AS calls_edge`;
//       const callsResult = await nebulaClient.executeNgql(callsQuery);
//       expect(callsResult.data).toBeDefined();
...
//     // 验证特殊字符的实体是否正确创建
//     try {
//       const specialQuery = `FETCH PROP ON ${ENTITY_TAG} "${entities[0].id}" YIELD vertex AS v`;
...
//     // 找到所有被 Home 页面依赖的组件
//     try {
//       const complexQuery = `GO FROM "Page:Home" OVER ${REL_IMPORTS} YIELD $$.${ENTITY_TAG}.raw_name AS name, $$.${ENTITY_TAG}.entity_type AS type`;
...
//     // 创建RAG实例并测试
//     const rag = new CodeRAG(nebulaClient);
...
//       expect(answer1.length).toBeGreaterThan(50); // 确保有有意义的回答
//     } catch (e) {
...
//       expect(answer2.length).toBeGreaterThan(50);
//     } catch (e) {
