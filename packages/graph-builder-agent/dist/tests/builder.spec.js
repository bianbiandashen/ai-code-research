...
(0, vitest_1.describe)('Builder - 真实数据库测试', () => {
        ...
        try {
            ...
            for (let i = 0; i < schema_1.CREATE_SCHEMA_STATEMENTS.length; i++) {
                ...
                try {
                    ...
                    if (statement.includes('CREATE SPACE') || statement.includes('USE ')) {
                        console.log('⏱️  等待图空间初始化...');
                        await new Promise(resolve => setTimeout(resolve, 2000)); // 等待2秒
                    }
                    // 如果是创建标签或边，也添加延迟确保 schema 生效
                    if (statement.includes('CREATE TAG') || statement.includes('CREATE EDGE')) {
    ...
    (0, vitest_1.it)('应该能通过RAG检索代码并回答问题', async () => {
        const files = [
            { id: 'src/pages/Login.vue', path: 'src/pages/Login.vue', name: 'Login.vue', extension: '.vue' },
            { id: 'src/pages/UserProfile.vue', path: 'src/pages/UserProfile.vue', name: 'UserProfile.vue', extension: '.vue' },
            { id: 'src/components/LoginForm.vue', path: 'src/components/LoginForm.vue', name: 'LoginForm.vue', extension: '.vue' },
            { id: 'src/components/UserCard.vue', path: 'src/components/UserCard.vue', name: 'UserCard.vue', extension: '.vue' },
            { id: 'src/services/authService.ts', path: 'src/services/authService.ts', name: 'authService.ts', extension: '.ts' },
            { id: 'src/services/userService.ts', path: 'src/services/userService.ts', name: 'userService.ts', extension: '.ts' },
            { id: 'src/utils/validation.ts', path: 'src/utils/validation.ts', name: 'validation.ts', extension: '.ts' },
            { id: 'src/stores/userStore.ts', path: 'src/stores/userStore.ts', name: 'userStore.ts', extension: '.ts' }
        ];
        const entities = [
            {
                id: 'Page:Login',
            ...
            },
            {
                ...
                CALLS: ['Service:userService.getUserInfo', 'Service:userService.updateProfile'],
                EMITS: [],
            ...
            },
            {
                ...
                CALLS: ['Util:validation.validateEmail', 'Util:validation.validatePassword'],
