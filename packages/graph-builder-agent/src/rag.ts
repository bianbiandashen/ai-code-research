...
export class CodeRAG {
  ...
  async query(question: string): Promise<string> {
    console.error("🔍 开始RAG检索...");

    // 1. 从问题中提取关键词进行图谱检索
    const keywords = this.extractKeywords(question);
    console.error(`📝 提取关键词: ${keywords.join(", ")}`);

    // 2. 在图谱中搜索相关实体（一跳）
    const relatedEntities = await this.searchEntities(keywords);
    console.error(`🎯 找到 ${relatedEntities.length} 个相关实体`);

    // 3. 进行二跳查询，找到更多相关信息
    const contextInfo = await this.expandContext(relatedEntities);
    console.error(`🔗 扩展后共获得 ${contextInfo.entities.length} 个实体信息`);
   ...
   * 在图谱中搜索相关实体
   */
  private async searchEntities(keywords: string[]): Promise<any[]> {
    const entities: any[] = [];
    ...
    for (const keyword of keywords) {
      try {
        ...
        for (const id of possibleIds) {
          try {
            const query = `FETCH PROP ON ${ENTITY_TAG} "${id}" YIELD vertex AS v`;
        ...
        for (const entityId of knownEntities) {
          if (entityId.toLowerCase().includes(keyword.toLowerCase())) {
            try {
              const query = `FETCH PROP ON ${ENTITY_TAG} "${entityId}" YIELD vertex AS v`;
              const result = await this.client.executeNgql(query);
        ...
        console.error(`⚠️  搜索关键词 "${keyword}" 时出错，继续其他搜索`);
      }
    }
    ...
    const uniqueEntities = entities.filter(
      (entity, index, self) =>
        index === self.findIndex((e) => e.v?._vid === entity.v?._vid)
    );
  ...
  private async expandContext(
    ...
    for (const entity of entities) {
      if (!entity.v?._vid) continue;

      try {
        // 查询导入关系
        const importsQuery = `GO FROM "${entity.v._vid}" OVER ${REL_IMPORTS} YIELD $$.${ENTITY_TAG}.raw_name AS name, $$.${ENTITY_TAG}.description AS desc, edge AS rel, dst(edge) AS target`;
        const importsResult = await this.client.executeNgql(importsQuery);

        if (importsResult.data?.length > 0) {
          relationships.push(
            ...importsResult.data.map((r) => ({
              ...r,
              type: "IMPORTS",
              source: entity.v._vid,
            }))
          );
          ...
          for (const rel of importsResult.data) {
            if (rel.target) {
              try {
                const targetQuery = `FETCH PROP ON ${ENTITY_TAG} "${rel.target}" YIELD vertex AS v`;
        ...
        // 查询调用关系
        const callsQuery = `GO FROM "${entity.v._vid}" OVER ${REL_CALLS} YIELD $$.${ENTITY_TAG}.raw_name AS name, $$.${ENTITY_TAG}.description AS desc, edge AS rel, dst(edge) AS target`;
        const callsResult = await this.client.executeNgql(callsQuery);

        if (callsResult.data?.length > 0) {
          relationships.push(
            ...callsResult.data.map((r) => ({
              ...r,
              type: "CALLS",
              source: entity.v._vid,
            }))
        ...
        // 查询被谁包含（反向查询）
        const containedQuery = `GO FROM "${entity.v._vid}" OVER ${REL_CONTAINS} REVERSELY YIELD $$.${FILE_TAG}.path AS file_path, edge AS rel, dst(edge) AS file_id`;
        const containedResult = await this.client.executeNgql(containedQuery);

        if (containedResult.data?.length > 0) {
          relationships.push(
            ...containedResult.data.map((r) => ({
              ...r,
              type: "CONTAINED_IN",
              source: entity.v._vid,
            }))
        ...
        console.error(`⚠️  扩展实体 ${entity.v._vid} 时出错，继续处理其他实体`);
      }
    }
   ...
   * 组织上下文信息
   */
  private organizeContext(contextInfo: {
    entities: any[];
    relationships: any[];
  }): string {
    ...
    for (const entity of contextInfo.entities) {
      if (entity.v) {
        ...
        context += `- **${props?.raw_name || "Unknown"}** (${
   ...
   * 调用AI回答问题
   */
  private async askAI(question: string, context: string): Promise<string> {
    try {
