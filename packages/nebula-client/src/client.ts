...
export class NebulaClient {
  ...
  private nebulaHost: string;
  private nebulaPort: number;
  ...
  constructor(options: NebulaClientOptions = {}) {
    this.gatewayHost =
      ...
      "http://192.168.132.44:8080";
    this.nebulaHost =
      options.nebulaHost || process.env.NEBULA_HOST || "10.4.44.78";
    this.nebulaPort =
      options.nebulaPort || parseInt(process.env.NEBULA_PORT || "9669");
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      if (result.code !== 0) {
        throw new Error(
          `Connect failed! code: ${result.code}, message: ${result.message}`
        );
      }
      ...
      if (cookies) {
        ...
        if (nsidMatch) {
          ...
          console.log(
            `Connected to Nebula Graph successfully with session ID: ${this.sessionId}`
          );
          return;
        }
      }
      ...
      if (result.data) {
        ...
        console.log(
          `Connected to Nebula Graph successfully with session ID from response body: ${this.sessionId}`
        );
      } else {
  ...
  async executeNgql(
    ...
    try {
      ...
      if (result.code !== 0) {
        ...
        throw new Error(
          `Gateway error! code: ${result.code}, message: ${result.message}`
        );
      }

      // 检查查询结果中是否有错误
      if (result.data && result.data.errors && result.data.errors.length > 0) {
        console.warn(`Query warning: ${result.data.errors[0].message}`, {
          query,
        });
      }
